...
Tier 2/3・配信モード（`monitor` / `turn` / `both` / `off`）・完全な対応表は [design.md §6](./design.md) / [§9](./design.md) を参照。運用補助として `agmsg version` / `agmsg upgrade [--check]`（GitHub Releases から最新版を取得して自身を置き換える）があります。
